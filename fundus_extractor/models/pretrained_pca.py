...
class SimCLR(object):
    def __init__(self, training_save_dir: str, backbone: nn.Module, optimizer: torch.optim.Optimizer,
                 scheduler: torch.optim.lr_scheduler.LRScheduler, device, batch_size: int, n_views: int,
                 temperature: float, similiarity_fn: str):
        self.device = device
        ...
        self.writer = SummaryWriter(training_save_dir)
        logging.basicConfig(filename=os.path.join(training_save_dir, 'training.log'), level=logging.DEBUG)

        self.criterion = torch.nn.CrossEntropyLoss().to(self.device)

        if similiarity_fn == 'dot':
            self.similarity = lambda x: torch.matmul(x, x.T)
        elif similiarity_fn == 'cosine':
            self.similarity = nn.CosineSimilarity()
        else:
            raise NotImplementedError(f'Similarity function {similiarity_fn} not implemented.')
...
def fit(dir_to_dataset: str, training_save_dir: str, n_views: int, batch_size: int, num_workers: int, img_size: int,
        out_dim: int, backbone_architecture: str, lr: float, weight_decay: float, **kwargs) -> None:
    assert n_views == 2, "Only two view training is supported. Please use --n-views 2."

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    transform = ContrastiveLearningViewGenerator(SimCLRAugmentations(img_size), n_views)
    dataset = Fundus_Left_Right_Combined_Dataset(dir_to_dataset, transform)
    ...
    simclr = SimCLR(training_save_dir, backbone, optimizer, scheduler, device, n_views=n_views, batch_size=batch_size,
                    **kwargs)
    simclr.train(train_loader, backbone_architecture, **kwargs)
